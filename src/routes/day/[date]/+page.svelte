<script lang="ts">
  ...
  let view = $state<'exercises' | 'categories' | 'exercisesInCategory'>('exercises');
  ...
  $effect(() => {
    const d = date;
    view = 'exercises';
    invoke<ExerciseWithSets[]>('get_workout_for_date', { date: d }).then((result) => {
      exercises = result;
    });
  });
...
>
  {#if view === 'exercises'}
    <div class="header">
      ...
      <div class="date-nav">
        ...
        <button class="date-label" onclick={() => goto(`/day/${todayStr()}`)}>{label}</button>
        <button class="date-nav-btn" onclick={() => navigateDay(1)}>›</button>
      </div>
    </div>
      ...
      <div class="list">
        {#each exercises as ex}
          <div class="exercise-card">
            <button class="exercise-card-header" onclick={() => goto(`/exercise/${ex.exercise_id}/${date}`)}>
  ...
  {:else if view === 'categories'}
    <div class="header">
      <button class="back-btn" onclick={() => (view = 'exercises')}>←</button>
